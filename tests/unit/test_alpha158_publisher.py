...
def test_single_alpha158_factor_publishes_an_independent_release(tmp_path) -> None:
    ...
    with duckdb.connect(str(database)) as connection:
        ...
        connection.execute("CREATE TABLE research.universe_daily(trade_date DATE, ts_code VARCHAR, eligible_for_signal BOOLEAN)")
        connection.execute(
            "CREATE TABLE research.market_daily(trade_date DATE, ts_code VARCHAR, open DOUBLE, high DOUBLE, low DOUBLE, close DOUBLE, volume_shares DOUBLE, amount_cny DOUBLE)"
        )
