...
- M4.6 已发布 RAW score 的成交约束证据，Explorer 对对应的 13 条 RAW 路径展示 Execution；其他变体保持 `NOT_AVAILABLE`。Explorer 只展示研究证据，不产生 Core Pool、模型特征永久淘汰或模型晋级结论。
