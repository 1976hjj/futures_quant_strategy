...
try {
    $arguments = @(
        ...
        '--output', 'data/tushare_financial_archive',
        '--page-size', '5000',
        '--sleep-ms', '100',
        '--min-free-gb', '30'
    )
    $archiveRoot = Join-Path $projectRoot 'data\tushare_financial_archive'
    ...
    $process = Start-Process `
        ...
        -PassThru
    Write-Host "M2-D financial backfill started. PID: $($process.Id)"
} finally {
